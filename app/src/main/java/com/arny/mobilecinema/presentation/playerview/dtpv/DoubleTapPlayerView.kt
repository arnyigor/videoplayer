...
package com.arny.mobilecinema.presentation.playerview.dtpv
...
open class DoubleTapPlayerView @JvmOverloads constructor(
    ...
    init {
        ...
        attrs?.let {
            context.withStyledAttributes(attrs, R.styleable.DoubleTapPlayerView, 0, 0) {
                controllerRef =
                    this.getResourceId(R.styleable.DoubleTapPlayerView_dtpv_controller, -1)
            }
        }
    }
     ...
     *
     * Primarily used for [YouTubeOverlay]
     */
    ...
    override fun onAttachedToWindow() {
        ...
        if (controllerRef != -1) {
            try {
                ...
                e.printStackTrace()
                Timber.tag("DoubleTapPlayerView")
                    .e("controllerRef is either invalid or not PlayerDoubleTapListener: ${e.message}")
            }
        }
    ...
    private class DoubleTapGestureListener(private val rootView: View) :
        ...
        private val mRunnable = Runnable {
            if (DEBUG) Timber.tag(TAG).d("Runnable called")
        ...
        override fun onSingleTapUp(e: MotionEvent): Boolean {
            if (isDoubleTapping) {
                if (DEBUG) Timber.tag(TAG).d("onSingleTapUp: isDoubleTapping = true")
        ...
        override fun onSingleTapConfirmed(e: MotionEvent): Boolean {
            ...
            if (DEBUG) Timber.tag(TAG).d("onSingleTapConfirmed: isDoubleTap = false")
            return rootView.performClick()
        }

        override fun onDoubleTap(e: MotionEvent): Boolean {
            // First tap (ACTION_DOWN) of both taps
            if (DEBUG) Timber.tag(TAG).d("onDoubleTap")
            if (!isDoubleTapping) {
                isDoubleTapping = true
                keepInDoubleTapMode()
        ...
        override fun onDoubleTapEvent(e: MotionEvent): Boolean {
            ...
            if (e.actionMasked == MotionEvent.ACTION_UP && isDoubleTapping) {
                if (DEBUG) Timber.tag(TAG).d("onDoubleTapEvent, ACTION_UP")
                controls?.onDoubleTapProgressUp(e.x, e.y)
                return true
            }
